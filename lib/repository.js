...
class Repository {
  constructor(dir, opts = {}) {
    ...
    if (opts.dry) {
      this.log('')
      this.log(yellow('new Repository'), dir, opts)
  ...
  write(name, data) {
    ...
    if (this.opts.dry) {
      this.log('')
      this.log(gray('(skip)'), red('writeFile:'), { cwd: this.dir, name, data })
    } else {
  ...
  exec(...args) {
    if (this.opts.dry) {
      ...
      if (ignored.indexOf(cmd) < 0) {
        ...
        this.log('')
        this.log(gray(hasEffects ? '(skip) ' : '') + color(`git ${cmd}:`), {
          cwd: this.dir,
